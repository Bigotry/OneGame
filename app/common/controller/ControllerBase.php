...
class ControllerBase extends Controller
{
    ...
    public function _initialize()
    {
        ...
        // 检查是否维护模式，维护模式测试号可正常访问
        $mid = is_login();
        
        if ($mid != SYS_ADMINISTRATOR_ID && $mid && config('sys_is_maintaining')) {
            
            $is_test = Db::name('member')->where(['id' => $mid])->value('is_test');
            
            empty($is_test) && $this->redirect('index/maintaining/index');
        }
    }
